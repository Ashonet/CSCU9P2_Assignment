...
public class ProductDatabase extends JFrame implements ActionListener
{
    ...
    public static void main(String[] args)
    {
        ...
        frame.setTitle("Product sales chart. Student no: 3115875");
        frame.createGUI();
   ...
   /**
    ...
    private void clearAllSales() {
        ...
        // Set all sales figures and percentages to zero
        for (int i = 1; i <= actualProducts; i++) {
            productSales[i] = 0;
            percentage[i] = 0;
        }
        
        // Repaint the display to reflect the changes
        repaint();
    }
    ...
    private void computePercentages() {
        ...
        if (totalSales == 0) {
            ...
            // Compute percentages for each product
            for (int i = 1; i <= actualProducts; i++) {
                percentage[i] = ((float) productSales[i] / totalSales) * 100;
            }
        }
    }
    ...
    public void updateSales(int productNumber, int prodSales) {
        // Update total sales by subtracting the old sales figure and adding the new one
        totalSales = totalSales - productSales[productNumber] + prodSales;
    
        // Update the sales figure for the specific product
        productSales[productNumber] = prodSales;
    
        // Recompute the percentages for all products
        computePercentages();
    }
    ...
    private void deleteProduct(int deleteNumber) {
        // Update total sales by subtracting the sales figure of the product to be deleted
        totalSales -= productSales[deleteNumber];
    
        // Shift remaining products and their sales figures to fill the gap
        for (int i = deleteNumber; i < actualProducts; i++) {
            productName[i] = productName[i + 1];
            productSales[i] = productSales[i + 1];
        }
    
        // Clear the data for the last product (now a duplicate of the one before it)
        productName[actualProducts] = null;
        productSales[actualProducts] = 0;
    
        // Decrement the count of actual products
        actualProducts--;
    
        // Recompute the percentages for all remaining products
        computePercentages();
    }
    ...
    private void sortByName() {
        // Call the recursive quick sort method | O(n log n)
        quickSort(1, actualProducts);
    }
    ...
    private int partition(int low, int high) {
        String pivot = productName[high];
        int i = low - 1;
    
        for (int j = low; j < high; j++) {
            if (productName[j].compareToIgnoreCase(pivot) <= 0) {
                i++;
                // Swap productName[i] and productName[j]
                String tempName = productName[i];
                productName[i] = productName[j];
                productName[j] = tempName;
    
                // Swap productSales[i] and productSales[j]
                int tempSales = productSales[i];
                productSales[i] = productSales[j];
                productSales[j] = tempSales;
    
                // Swap percentage[i] and percentage[j]
                float tempPercentage = percentage[i];
                percentage[i] = percentage[j];
                percentage[j] = tempPercentage;
            }
        }
    
        // Swap productName[i+1] and productName[high] (pivot)
        String tempName = productName[i + 1];
        productName[i + 1] = productName[high];
        productName[high] = tempName;
    
        // Swap productSales[i+1] and productSales[high] (pivot)
        int tempSales = productSales[i + 1];
        productSales[i + 1] = productSales[high];
        productSales[high] = tempSales;
    
        // Swap percentage[i+1] and percentage[high] (pivot)
        float tempPercentage = percentage[i + 1];
        percentage[i + 1] = percentage[high];
        percentage[high] = tempPercentage;
    ...
    {
        // Call the recursive quick sort method | O(n log n)
        quickSortSales(1, actualProducts);
    }
    private void quickSortSales(int low, int high) {
        if (low < high) {
            // Partition the array and get the pivot index
            int pivotIndex = partitionSales(low, high);
            // Recursively sort elements before and after the pivot
            quickSortSales(low, pivotIndex - 1);
            quickSortSales(pivotIndex + 1, high);
        }
    }
    private int partitionSales(int low, int high) {
        int pivot = productSales[high];
        int i = low - 1;
    
        for (int j = low; j < high; j++) {
            if (productSales[j] >= pivot) {
                i++;
                // Swap productName[i] and productName[j]
                String tempName = productName[i];
                productName[i] = productName[j];
